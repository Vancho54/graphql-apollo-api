...
const { isAuth, isTaskOwner } = require('./middleware')
const { combineResolvers,} = require('graphql-resolvers')
const { stringToBase64, base64ToString } = require('../helper')

module.exports = {
    Query: {
        tasks: combineResolvers(isAuth, async (_, { cursor, limit = 10 }, { loggedInUserId }) => {
            try {
        ...
        task: combineResolvers(isAuth, isTaskOwner, async (_, { id }) => {
            try {
    ...
    Mutation: {
        ...
        updateTask: combineResolvers(isAuth, isTaskOwner, async (_, { id, input })=> {
            try {
        ...
        deleteTask: combineResolvers(isAuth, isTaskOwner, async (_, { id }, { loggedInUserId }) => {
            try {
